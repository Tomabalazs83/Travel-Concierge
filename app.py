...
async def chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        prompt = f"You are a sophisticated British Butler. Address the user as Sir. Answer: {user_text}"
        response = ai_brain.generate_content(prompt)
        await update.message.reply_text(response.text)
    except Exception as e:
        logger.error(f"Chat Error: {e}")
        await update.message.reply_text(f"I'm dreadfully sorry, Sir, my thoughts are a bit muddled. (Note: {str(e)[:40]})")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("The Concierge is at your service. Use /check or simply speak to me, Sir.")
